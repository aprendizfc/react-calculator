...
function requestHandler(request, response) {
  ...
  fs.exists(requestedResource, function(exists) {
    ...
    // Finally, we read the requested file (asynchronously) and sen it's
    ...
    fs.readFile(
      ...
      function(err, file) { // callback function to handle end of file reading
        ...
        response.writeHead(200, headers); // write resopnse header (if any)
...
// create an instance for our httpServer and passing in our request handler 
// callback
...
server.listen(portNumber, function() {
  // log out to our console, so we know our server is up and running.
